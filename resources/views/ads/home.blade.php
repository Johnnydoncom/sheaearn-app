<x-app-layout>
    ...
    <div class="py-12">
        <div class="container">
            <h2 class="text-3xl font-semibold">{{ __('Sponsored Ads') }}</h2>
            <div class="divider mt-0"></div>
            <div class="grid sm:grid-cols-4 gap-4 grid-cols-1">
                <div class="col-span-3 grid grid-cols-1 gap-4">
                    @foreach($ads as $ad)
                        @include('partials.ads.style1', ['ad'=>$ad])
                    @endforeach
                </div>
                <div class="col-span-1 hidden sm:block">
                    <form action="{{route('blog.index')}}" class="mb-10">
                        <div class="form-control flex-row">
                            <input type="text" name="s" class="input input-bordered w-full rounded-none" placeholder="Search">
                            <button class="btn btn-secondary rounded-none"><x-cui-cil-search class="w-5 h-5" /></button>
                        </div>
                    </form>

                    <h3 class="font-bold text-lg sm:text-xl">Latest Posts</h3>
                    <div class="divider my-0"></div>
                    <div class="grid grid-cols-1 gap-2">
                        @foreach ($latestPosts as $latest)
                            <div class="flex flex-row gap-2">
                                <img class="w-full h-10 sm:h-20 object-cover sm:w-20 rounded-none" src="{{ $latest->featured_img_thumb }}" alt="{{$latest->title}}" />

                                <div class="flex flex-col justify-start">
                                    <h5 class="text-gray-900 text-sm font-medium line-clamp-2"><a href="{{$latest->entry_url}}">{{$latest->title}}</a></h5>

                                    <div class="flex items-center justify-start gap-2">
                                        <a href="{{$latest->entry_url}}">
                                            <p class="text-gray-500 text-xs flex items-center gap-2">
                                                ...
                                                <span>{{ \Str::readDuration($latest->title, $latest->description). ' min read' }}</span></p>
                                        </a>
                                    </div>
                                </div>
                            </div>
